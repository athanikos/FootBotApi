...
@app.route("/api/v1/matches/<int:league_id>/<int:team_id>/<before_date>/<time_status>", methods=['GET'])
...
@app.route("/api/v1/stats/<int:league_id>/<int:team_id>/<before_date>/<time_status>", methods=['GET'])
...
def fetch_flat_matches(before_date, league_id, team_id, time_status):
    ...
    items = flatmatches.objects((Q(localteam_id=team_id) | Q(visitorteam_id=team_id)) & Q(time_status=time_status)
                          & Q(league_id=league_id) & Q(time_starting_at_date__lte=before_date)).order_by('time_starting_at_date-')[:10]
    app.logger.info(league_id)
    app.logger.info(team_id)
    app.logger.info(before_date)
    app.logger.info(time_status)
